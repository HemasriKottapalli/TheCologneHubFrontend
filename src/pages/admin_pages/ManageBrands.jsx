...
export default function ManageBrands() {
  ...
  const handleAddBrand = async (e) => {
    ...
    try {
      ...
      setBrands([
        ...brands,
        { id: data.brand._id || data.brand.id, name: data.brand.brandName ?? data.brand.name },
      ]);
      resetForm();
    } catch (err) {
  ...
  const handleEditBrand = async (e) => {
    ...
    try {
      ...
      setBrands(
        brands.map((b) =>
          b.id === editingBrand.id ? { ...b, name: data.brand.brandName ?? data.brand.name } : b
        )
      );
      resetForm();
    } catch (err) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-2xl sm:text-3xl font-bold text-[#8B5A7C] mt-1">{brands.length}</p>
            </div>
            <button
              onClick={() => {
                setShowAddForm(true);
              ...
              disabled={loading}
              className="w-full sm:w-auto bg-[#8B5A7C] hover:bg-[#7A4E6C] text-white px-4 py-3 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Plus size={20} />
              Add New Brand
            </button>
              ...
              <div className="p-4 sm:p-6">
                ...
                <div>
                  <div className="mb-6">
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          editingBrand ? handleEditBrand(e) : handleAddBrand(e);
                        }
                      }}
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full bg-[#8B5A7C] hover:bg-[#7A4E6C] text-white px-3 py-3 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Save size={16} />
                ...
                {filteredBrands.map((brand, index) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:grid sm:grid-cols-12 gap-3 sm:gap-4 items-start sm:items-center">
                      <div className="sm:col-span-10 flex items-center gap-3">
                      ...
                      <div className="sm:col-span-2 flex gap-2">
                        <button
                          ...
                          disabled={loading}
                          className="p-2 text-gray-600 hover:text-[#8B5A7C] hover:bg-[#8B5A7C]/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Edit brand"
                        >
                          <Edit2 size={16} />
                        </button>
